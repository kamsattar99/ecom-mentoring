...
const ShopifyIcon = () => (
  <svg width="44" height="50" viewBox="0 0 109.5 124.5" aria-hidden="true" className="h-[42px] w-[38px] sm:h-[50px] sm:w-[44px]">
    ...
    <path fill="#FFF" d="M58.1 39.6l-3.8 14.3s-4.3-1.9-9.3-1.6c-7.4.5-7.5 5.1-7.4 6.3.4 6.4 17.2 7.8 18.1 22.7.7 11.8-6.2 19.8-16.3 20.4-12.1.8-18.7-6.4-18.7-6.4l2.6-10.9s6.7 5 12 4.7c3.5-.2 4.8-3.1 4.6-5.1-.5-8.3-14.2-7.8-15.1-21.5-.7-11.5 6.8-23.1 23.5-24.2 6.5-.4 9.8 1.3 9.8 1.3z" />
  </svg>
);
...
export default function HeroSection() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (bannerRef.current) {
        const tiltX = HOME_FEATURES.heroTilt && window.innerWidth > 860 ? pointerY * -3.5 : 0;
        const tiltY = HOME_FEATURES.heroTilt && window.innerWidth > 860 ? pointerX * 4.5 : 0;
        bannerRef.current.style.transform = `translate3d(0, ${scroll * -44}px, 0) scale(${1 + scroll * 0.06}) rotateX(${tiltX}deg) rotateY(${tiltY}deg)`;
        bannerRef.current.style.setProperty("--highlight-x", `${50 + pointerX * 18}%`);
        bannerRef.current.style.setProperty("--highlight-y", `${30 + pointerY * 18}%`);
      }
      section.querySelectorAll<HTMLElement>("[data-hero-object]").forEach((object) => {
    ...
    const onScroll = () => {
      const rect = section.getBoundingClientRect();
      target.scroll = Math.min(Math.max(-rect.top / (section.offsetHeight * 0.95), 0), 1);
      schedule();
    };
  ...
  return (
    ...
    >
      <div aria-hidden="true" className="pointer-events-none absolute inset-0 overflow-hidden bg-[#0F0E13]">
        <div ref={backgroundRef} className="absolute -inset-[3%] will-change-transform">
          <picture>
            <source media="(max-width: 860px)" srcSet={homeAssets.heroEnvironmentMobile} />
            <img
        ...
        <div ref={gridRef} className="home-grid-overlay absolute inset-0 will-change-transform" />
      ...
      <div ref={copyRef} className="relative z-[4] flex w-[min(calc(100%-40px),900px)] max-w-[calc(100%-40px)] flex-col items-center text-center will-change-transform">
        <div className={`hero-entry mb-7 inline-flex items-center gap-2.5 rounded-full border border-[#B394F2]/30 bg-[#0F0E13]/55 py-2 pl-2.5 pr-3.5 backdrop-blur-md ${entered ? "is-entered" : ""}`} style={{ transitionDelay: "220ms" }}>
          <span className="home-status-pulse h-[7px] w-[7px] rounded-full bg-[#B394F2] shadow-[0_0_0_4px_rgba(179,148,242,0.18)]" />
          <span className="text-[11px] font-semibold tracking-[0.04em] text-white/80 sm:text-[12px]">6-week, hands-on AI ecommerce mentorship</span>
        </div>
        ...
        <h1 className="mb-6 font-serif text-[clamp(44px,6.8vw,86px)] font-normal leading-[0.96] tracking-[-0.035em] text-white [text-shadow:0_20px_60px_rgba(0,0,0,0.5)]">
          <span className="block overflow-hidden pb-[0.12em] -mb-[0.06em]"><span className={`hero-line ${entered ? "is-entered" : ""}`} style={{ transitionDelay: "320ms" }}>Stop Guessing</span></span>
          <span className="block overflow-hidden pb-[0.12em] -mb-[0.06em]"><span className={`hero-line ${entered ? "is-entered" : ""}`} style={{ transitionDelay: "410ms" }}>What To <em className="italic text-[#B394F2]">Test Next.</em></span></span>
        </h1>

        <p className={`hero-entry mb-3.5 max-w-[640px] text-[clamp(16px,1.4vw,19px)] leading-[1.6] text-white/80 ${entered ? "is-entered" : ""}`} style={{ transitionDelay: "500ms" }}>
        ...
        <p className={`hero-entry mb-8 max-w-[560px] text-[14px] leading-[1.7] text-white/50 ${entered ? "is-entered" : ""}`} style={{ transitionDelay: "560ms" }}>
        ...
        <div className={`hero-entry flex flex-col items-stretch justify-center gap-3 sm:flex-row sm:items-center ${entered ? "is-entered" : ""}`} style={{ transitionDelay: "620ms" }}>
          <a href="/apply" className="btn-primary justify-center px-8 py-[17px] text-[15px] font-semibold">
            Apply For A Fit Call <ArrowIcon />
          </a>
          <a href="#results" className="btn-ghost justify-center bg-[#0F0E13]/40 px-6 py-4 backdrop-blur-md">
            <PlayIcon /> Watch Student Interviews
          </a>
        </div>
        <p className={`hero-entry mt-[18px] text-[12px] leading-[1.6] text-white/45 ${entered ? "is-entered" : ""}`} style={{ transitionDelay: "700ms" }}>
          Around 2 minutes to apply · Reviewed before a call is offered · Results are not guaranteed
        </p>

        <div className={`hero-entry mt-5 grid w-full max-w-[520px] grid-cols-3 gap-3 border-t border-white/10 pt-4 ${entered ? "is-entered" : ""}`} style={{ transitionDelay: "760ms" }}>
          {[["600K+", "YouTube community"], ["5", "Full student interviews"], ["2", "Seven-figure mentors"]].map(([value, label]) => (
            <div key={label}>
              <p className="font-serif text-[22px] leading-none text-white sm:text-[26px]">{value}</p>
              <p className="mt-1.5 text-[9px] leading-[1.3] text-white/45 sm:text-[11px]">{label}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="relative z-[3] mt-auto flex w-[min(calc(100%-32px),1180px)] justify-center pt-[clamp(36px,6vh,64px)]">
        {HOME_FEATURES.heroObjects && (
          <>
            <img data-hero-object data-depth="10" src={homeAssets.boxIcon} alt="" width={900} height={900} decoding="async" className="home-desktop-only pointer-events-none absolute bottom-[-8%] left-[2%] z-[2] w-[clamp(150px,16%,210px)] will-change-transform" />
            <img data-hero-object data-depth="12" src={homeAssets.adsIcon} alt="" width={480} height={480} decoding="async" className="home-desktop-only pointer-events-none absolute bottom-[-14%] left-[13%] z-[6] w-[clamp(80px,8%,110px)] will-change-transform" />
            <img data-hero-object data-depth="8" src={homeAssets.aiIcon} alt="" width={480} height={480} decoding="async" className="home-desktop-only pointer-events-none absolute right-[1%] top-[-4%] z-[6] w-[clamp(110px,13%,160px)] will-change-transform" />
          </>
        )}

        <div ref={bannerRef} className="w-[min(92vw,760px)] max-w-full [transform-style:preserve-3d] will-change-transform">
          <div className={`hero-banner-entry ${entered ? "is-entered" : ""}`} style={{ transitionDelay: "560ms" }}>
            <a
              href="#results"
            ...
            >
              ...
              <div className="relative flex min-h-[140px] flex-wrap items-center gap-4 px-[clamp(18px,2.6vw,32px)] py-[clamp(18px,2.2vw,26px)] sm:gap-7">
                <div className="relative flex h-[52px] w-[52px] shrink-0 items-center justify-center rounded-[18px] border border-[#17131E]/10 bg-white shadow-[0_10px_24px_rgba(23,19,30,0.1)] sm:h-[76px] sm:w-[76px] sm:rounded-[20px]">
                  <ShopifyIcon />
                  <span aria-hidden="true" className="home-status-pulse absolute -right-1 -top-1 h-3.5 w-3.5 rounded-full border-2 border-white bg-[#95BF47]" />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="m-0 text-[10px] font-bold uppercase tracking-[0.14em] text-[#6E6879] sm:text-[11px]">Shopify · Student result</p>
                  <div className="mt-1.5 flex flex-wrap items-baseline gap-x-3 gap-y-0">
                    <span aria-label="$7,042,738.43" className="whitespace-nowrap font-serif text-[clamp(28px,7vw,54px)] leading-none tracking-[-0.03em] text-[#17131E] tabular-nums">{formattedSales}</span>
                    <span className="whitespace-nowrap text-[14px] font-semibold text-[#3C3646] sm:text-[17px]">Total Store Sales</span>
                  </div>
                  <p className="mt-2 text-[11px] leading-[1.3] text-[#6E6879] sm:text-[12px]">Student-provided · 365 days</p>
                </div>
      ...
      <div aria-hidden="true" className={`hero-entry home-desktop-only absolute bottom-8 right-10 z-[5] flex items-center gap-3 text-[11px] font-semibold uppercase tracking-[0.14em] text-white/45 ${entered ? "is-entered" : ""}`} style={{ transitionDelay: "1400ms" }}>
        <span className="home-scroll-pulse block h-11 w-px bg-[linear-gradient(180deg,transparent,#B394F2)]" /> Scroll
      </div>
    </section>
  );
}
